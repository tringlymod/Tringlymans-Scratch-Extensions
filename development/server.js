...
app.listen(PORT, (info) => {
	function serverType() {
		const node = "--server-type:node";
		const nodemon = "--server-type:nodemon";
		if (process.argv.includes(node)) {
			return process.argv[process.argv.indexOf(node)].replace(
				"--server-type:",
				""
			);
		}
		if (process.argv.includes(nodemon)) {
			return process.argv[process.argv.indexOf(nodemon)].replace(
				"--server-type:",
				""
			);
		}
	...
	console.log(
		(function (...msg) {
			const highestLength = msg.reduce((pre, curr) =>
				pre > curr ? pre : curr
			).length;

			const padding = `+${"".padEnd(highestLength + 2, "-")}+`;

			return `\n${padding}\n${msg.map((MSG) => `| ${MSG ?? ""}${"".padEnd(highestLength - (MSG !== null ? MSG.length : 0) + 1, " ")}|`).join("\n")}\n${padding}`;
		})(
			/* use null or "" for empty lines */
