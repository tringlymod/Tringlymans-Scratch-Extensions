...
/**
 * @type {import("../extensions/settings/extension.config.json")}
 */
const extensionSettings = ExtendedJSON.parse(
	fs.readFileSync(
		pathUtil.join(__dirname, "../extensions/settings/extension.config.json"),
		"utf-8"
	)
);
...
class HomepageFile extends BuildFile {
	...
	getFullExtensionURL(extensionSlug) {
		return `${this.host}${extensionSettings.URLpath.exts.startsWith}${extensionSlug}${extensionSettings.URLpath.exts.endsWith}`;
	}

	getDocumentationURL(extensionSlug) {
		return `${this.host}${extensionSettings.URLpath.docs.startsWith}${extensionSlug}${extensionSettings.URLpath.docs.endsWith}`;
	}
	...
	getTagColors(tag) {
		return extensionSettings.metadata.tagColors[tag];
	}

	getImageURL(extensionSlug, extensionImages, metadataColor = "#0fbd8c") {
		/**
		 * @type {string}
		 */
		const imagePath = extensionImages[extensionSlug];
		function unknown() {
			const image = pathUtil.join(__dirname, "../images/ext/unknown.svg");
			return `data:image/svg+xml,${encodeURIComponent(
				fs.readFileSync(image, "utf-8").replaceAll("#0fbd8c", metadataColor)
			)}`;
		}
	...
	getRunSampleURL(sampleFile) {
		...
		const urlParam = `project_url=${this.host}${path}`;
		return `studio.penguinmod.com/editor.html?${urlParam}`;
	}
	...
	read() {
		...
		return renderTemplate(this.sourcePath, {
			...
			getTagColors: this.getTagColors.bind(this),
...
class SVGFile extends ImageFile {
	validate() {
		...
		if (contents.includes("<text")) {
			...
			return `SVG must not contain <text> elements -- please convert the text to a path. This ensures it will display correctly on all devices.${Colors.RESET}`;
		}
...
class Builder {
	...
	constructor(mode) {
		...
		this.host = new HomepageFile("", "", "", "", "", this.mode).host;

		this.extensionsRoot = pathUtil.join(
			__dirname,
			`../extensions/${extensionSettings.basePath}`
		);
		this.websiteRoot = pathUtil.join(__dirname, "../website");
		this.imagesRoot = pathUtil.join(
			__dirname,
			`../images/${extensionSettings.basePath}`
		);
		this.docsRoot = pathUtil.join(
			__dirname,
			`../docs/${extensionSettings.basePath}`
		);
		this.samplesRoot = pathUtil.join(
			__dirname,
			`../samples/${extensionSettings.basePath}`
		);
	...
	build() {
		...
		const featuredExtensionSlugs = ExtendedJSON.parse(
			fs.readFileSync(
				pathUtil.join(this.extensionsRoot, "../settings/extensions.json"),
				"utf-8"
			)
		);
		...
		for (const [filename, absolutePath] of recursiveReadDirectory(
			...
			build.files[
				`/${extensionSettings.URLpath.exts.startsWith}${extensionSlug}${extensionSettings.URLpath.exts.endsWith}`
			] = file;
		}
		...
		for (const [filename, absolutePath] of recursiveReadDirectory(
			...
			build.files[
				`/${extensionSettings.URLpath.docs.startsWith}${extensionSlug}${extensionSettings.URLpath.docs.endsWith}`
			] = file;
		}
	...
	validate() {
		...
		for (const [fileName, file] of Object.entries(build.files)) {
			try {
				if (file.validate()) {
					warns.push({ fileName, error: file.validate() });
				}
			} catch (e) {
				errors.push({
					fileName: fileName,
